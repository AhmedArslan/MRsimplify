...
#Step-1: READ Exposure data:
  ...
  try(resPRESSO_munge <- run_mr_presso(harmonise_munge))
  try(resPRESSO_munge <- data.frame(resPRESSO_munge = unlist(resPRESSO_munge)))
  try(write.table(resPRESSO_munge, file = paste(file_name,"/", 'resPRESSO_munge.txt', sep='') ,sep='\t', quote=FALSE, col.names=TRUE, row.names=FALSE, append=TRUE))
  #print(resPRESSO_munge) 
  #Combine all results
  ...
  print_color("creating forest plot... ", "blue")
  print("\n")
  res_single <- mr_singlesnp(harmonise_munge)
  pdf(file=paste(file_name,"/","p2.pdf", sep= ""), width = 9, height = 7)
  mr_forest_plot(res_single)
  dev.off()
  print_color("creating leaveoneout plots... ", "blue")
  res_loo <- mr_leaveoneout(harmonise_munge)
  pdf(file=paste(file_name,"/","p3.pdf", sep= ""), width = 9, height = 7)
